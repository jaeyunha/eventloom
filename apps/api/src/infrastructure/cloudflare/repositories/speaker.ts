  ...
  implements SpeakerAccountWorkloadRepository, SpeakerOrganizerLifecycleRepository
{
    ...
    const rows = await this.#db
      ...
      .prepare(
        `SELECT pg.organization_id, pg.participant_id, pg.permissions_json,
                max(CASE WHEN sp.role = 'primary' THEN 1 ELSE 0 END) AS is_primary,
    ...
    const capabilitiesByParticipant = Object.fromEntries(
      matches.map((row) => {
        ...
        if (Number(row.is_primary) === 1) {
          capabilities.push("submission-edit", "roster-manage");
        }
    ...
    const rows = await this.#db
      ...
      .prepare(
        `SELECT e.organization_id, e.id AS event_id, e.name, e.slug,
                ...
                max(CASE WHEN sp.role = 'primary' THEN 1 ELSE 0 END)
                  AS is_primary,
                group_concat(DISTINCT sp.submission_id) AS submission_ids
           FROM participant_grants pg
           JOIN event_role_invitations invitation
    ...
    return (rows.results ?? []).map((row) => {
      ...
      return {
        ...
        capabilities: [
          ...
          "asset-write",
          "asset-comment",
          ...(Number(row.is_primary) === 1 ? (["submission-edit", "roster-manage"] as const) : []),
          "resource-read",
        ],
  ...
  async listRosterForEvent(
    ...
    const rows = await this.#db
      ...
      .bind(eventId)
      .all<Record<string, unknown>>();
    const profileEntries = (rows.results ?? []).map((row) => {
    ...
    const explicitRows = await this.#db
      .prepare(
        `SELECT *
           FROM speaker_roster
          WHERE event_id = ?
      ...
      .bind(eventId)
      .all<Record<string, unknown>>();
    const explicitEntries = (explicitRows.results ?? []).map((row) => ({
      id: String(row.id),
      eventId,
      ...
      company: String(row.company),
      biography: String(row.biography),
      ...(row.social_links_json === null
        ? {}
        : {
            socialLinks: JSON.parse(String(row.social_links_json)) as Record<string, string>,
          }),
      ...(row.travel_logistics_json === null
        ? {}
        : {
            travelLogistics: JSON.parse(
              String(row.travel_logistics_json),
            ) as import("../../../features/speaker/types").SpeakerTravelLogistics,
          }),
      ...(row.headshot_asset_id === null ? {} : { headshotAssetId: String(row.headshot_asset_id) }),
      ...(row.source_type === null
        ? {}
        : { sourceType: row.source_type as "cfp" | "manual" | "csv" | "crm" }),
      ...(row.source_id === null ? {} : { sourceId: String(row.source_id) }),
      role: String(row.role) === "primary" ? ("primary" as const) : ("co_speaker" as const),
      status:
        String(row.status) === "revoked"
          ? ("revoked" as const)
          : String(row.status) === "active"
            ? ("active" as const)
            : ("pending" as const),
      ...(row.workflow_status === null ? {} : { workflowStatus: String(row.workflow_status) }),
      ...(row.organizer_status === null ? {} : { organizerStatus: String(row.organizer_status) }),
      version: Number(row.version),
      createdAt: String(row.created_at),
      updatedAt: String(row.updated_at),
      ...(row.author_account_id === null ? {} : { authorAccountId: String(row.author_account_id) }),
    }));
    const merged = new Map<string, import("../../../features/speaker/types").SpeakerRosterEntry>();
    for (const entry of profileEntries) {
      merged.set(`${d1SubmissionId(entry.submissionId ?? "")}:${entry.participantId}`, entry);
    }
    for (const entry of explicitEntries) {
      const key = `${d1SubmissionId(entry.submissionId ?? "")}:${entry.participantId}`;
      const existing = merged.get(key);
      const entryUsesD1SubmissionId =
        entry.submissionId !== undefined &&
        entry.submissionId === d1SubmissionId(entry.submissionId);
      const existingUsesD1SubmissionId =
        existing?.submissionId !== undefined &&
        existing.submissionId === d1SubmissionId(existing.submissionId);
      if (existing === undefined || entryUsesD1SubmissionId || !existingUsesD1SubmissionId) {
  ...
  async saveRoster(
    entry: import("../../../features/speaker/types").SpeakerRosterEntry,
    expectedVersion: number | null,
  ): Promise<RepositoryResult<import("../../../features/speaker/types").SpeakerRosterEntry>> {
    const submissionId = d1SubmissionId(entry.submissionId ?? "");
    if (submissionId.length === 0) return { ok: false, reason: "invalid_state" };
    const eventRows = await this.#db
      .prepare("SELECT organization_id FROM events WHERE id = ? ORDER BY organization_id LIMIT 2")
      .bind(entry.eventId)
      .all<{ organization_id: string }>();
    const eventMatches = eventRows.results ?? [];
    const event = eventMatches[0];
    if (eventMatches.length !== 1 || event === undefined) {
    ...
    const current = await this.#db
      ...
      .bind(organizationId, entry.eventId, submissionId, entry.participantId)
      .first<{ version: number; status: string; created_at: string }>();
    if (expectedVersion !== null) {
      if (current === null) return { ok: false, reason: "not_found" };
      if (current.version !== expectedVersion) {
        return { ok: false, reason: "version_conflict" };
      }
    } else if (current !== null && current.status !== "revoked") {
      return { ok: false, reason: "version_conflict" };
    }
      ...
      .prepare(
        `SELECT id, normalized_email
          ...
          WHERE organization_id = ? AND event_id = ?
            AND (id = ? OR normalized_email = ? COLLATE NOCASE)
       ORDER BY id`,
      )
      .bind(organizationId, entry.eventId, entry.participantId, email)
      .all<{ id: string; normalized_email: string | null }>();
    if (
      (identityRows.results ?? []).some(
        (identity) =>
          identity.id !== entry.participantId ||
          (identity.normalized_email !== null && identity.normalized_email.toLowerCase() !== email),
      )
    ) {
      return { ok: false, reason: "invalid_state" };
    }
    const nameParts = entry.displayName.trim().split(/\s+/u);
    const firstName = nameParts[0] ?? entry.displayName.trim();
    const lastName = nameParts.slice(1).join(" ");
    const createdAt = current?.created_at ?? entry.createdAt;
    const persistedVersion = current === null ? 1 : current.version + 1;
    const sourceType = entry.sourceType ?? "manual";
    const sourceId = entry.sourceId ?? `roster:${submissionId}:${email}`;
    await this.#db
      .prepare(
        `INSERT INTO participants
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?,?,NULL,1,?,?)
         ON CONFLICT(organization_id,event_id,id) DO NOTHING`,
      )
      .bind(
        entry.participantId,
        organizationId,
      ...
      .run();
    const rosterStatement =
      current === null
        ? this.#db
            .prepare(
              `INSERT INTO speaker_roster
                 (id,organization_id,event_id,submission_id,participant_id,role,status,
                  workflow_status,organizer_status,display_name,email,job_title,company,biography,
                  social_links_json,travel_logistics_json,headshot_asset_id,source_type,source_id,
                  version,created_at,updated_at,author_account_id)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
               ON CONFLICT(organization_id,event_id,submission_id,participant_id) DO NOTHING`,
            )
            .bind(
              entry.id,
              organizationId,
              ...
              entry.role,
              entry.status,
              entry.workflowStatus ?? entry.status,
              entry.organizerStatus ?? entry.status,
              entry.displayName,
              email,
              entry.jobTitle ?? "",
              entry.company ?? "",
              entry.biography ?? "",
              json(entry.socialLinks ?? {}),
              json(entry.travelLogistics ?? {}),
              entry.headshotAssetId ?? null,
              sourceType,
              sourceId,
              persistedVersion,
              createdAt,
            ...
            .prepare(
              `UPDATE speaker_roster
                ...
                WHERE organization_id = ? AND event_id = ? AND submission_id = ?
                  ...
                  ${expectedVersion === null ? "AND status = 'revoked'" : ""}`,
            )
            .bind(
              entry.id,
              entry.role,
    ...
    const rosterResult = await rosterStatement.run();
    if (rosterResult.meta.changes !== 1) {
      return { ok: false, reason: "version_conflict" };
    }
    const profileStatus =
      entry.status === "active" ? "confirmed" : entry.status === "revoked" ? "revoked" : "pending";
    const sessionId = `session-${submissionId}`;
    await this.#db.batch([
      this.#db
        .prepare(
          `INSERT INTO speaker_profiles
           ...
           ON CONFLICT(organization_id,event_id,participant_id) DO NOTHING`,
        )
        .bind(
          `speaker-profile:${organizationId}:${entry.eventId}:${entry.participantId}`,
          organizationId,
          entry.eventId,
          entry.participantId,
          entry.displayName,
      ...
      this.#db
        .prepare(
          `INSERT INTO submission_participants
           ...
           VALUES (?,?,?,?,?,?,?,COALESCE((
             SELECT max(ordinal) + 1 FROM submission_participants
              WHERE organization_id = ? AND event_id = ? AND submission_id = ?
           ),0))
           ON CONFLICT(organization_id,submission_id,participant_id) DO UPDATE SET
             role = excluded.role,
      ...
      this.#db
        .prepare(
          `INSERT INTO session_speakers
             (organization_id,event_id,session_id,ordinal,speaker_id,display_name,role)
           SELECT ?,?,?,COALESCE((
                    SELECT max(ordinal) + 1 FROM session_speakers
                     WHERE organization_id = ? AND event_id = ? AND session_id = ?
                  ),0),?,?,?
            WHERE EXISTS (
              SELECT 1 FROM sessions
               WHERE organization_id = ? AND event_id = ? AND id = ? AND deleted_at IS NULL
            )
           ON CONFLICT(organization_id,event_id,session_id,speaker_id) DO UPDATE SET
  ...
  async revokeRoster(
    ...
    const canonicalSubmissionId = d1SubmissionId(submissionId);
    const current = await this.#db
      .prepare(
        `SELECT organization_id, role, version
           FROM speaker_roster
          WHERE event_id = ? AND submission_id = ? AND participant_id = ?
       ORDER BY organization_id
          LIMIT 2`,
      )
      .bind(eventId, canonicalSubmissionId, participantId)
      .all<{ organization_id: string; role: string; version: number }>();
    const matches = current.results ?? [];
    const row = matches[0];
    if (matches.length !== 1 || row === undefined) return { ok: false, reason: "not_found" };
    if (row.role === "primary") return { ok: false, reason: "invalid_state" };
    if (row.version !== expectedVersion) return { ok: false, reason: "version_conflict" };
    ...
    const update = await this.#db
      ...
      .run();
    if (update.meta.changes !== 1) return { ok: false, reason: "version_conflict" };
    const sessionId = `session-${canonicalSubmissionId}`;
    await this.#db.batch([
      this.#db
        .prepare(
          `DELETE FROM submission_participants
            WHERE organization_id = ? AND event_id = ? AND submission_id = ?
              AND participant_id = ? AND role = 'co_speaker'`,
        )
        .bind(row.organization_id, eventId, canonicalSubmissionId, participantId),
      this.#db
        .prepare(
          `DELETE FROM session_speakers
            WHERE organization_id = ? AND event_id = ? AND session_id = ? AND speaker_id = ?`,
        )
        .bind(row.organization_id, eventId, sessionId, participantId),
      this.#db
        .prepare(
          `UPDATE event_role_invitations
            ...
            WHERE organization_id = ? AND event_id = ? AND participant_id = ?
              AND role = 'speaker' AND status = 'accepted'
              AND NOT EXISTS (
                SELECT 1
                  FROM submission_participants AS participant
                  JOIN evaluation_decisions AS decision
                    ON decision.organization_id = participant.organization_id
                   AND decision.event_id = participant.event_id
                   AND decision.submission_id = participant.submission_id
    ...
    const entries = await this.listRoster(eventId, canonicalSubmissionId);
