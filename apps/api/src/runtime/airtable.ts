...
class D1PublishedSpeakerProjectionStore implements PublishedSpeakerRouteDependencies {
  constructor(
    ...
    private readonly publications: D1RuntimeDependencies["programPublication"],
...
export class AirtableSpeakerRepository implements SpeakerRepository {
  ...
  async getOrganizerReadModel(
    ...
    const eventPromise = this.#events.find(eventId);
    const membershipPromise = this.#database
      .prepare(
        `SELECT organization_id, role
           FROM organization_memberships
          WHERE user_id = ?
      ...
      .bind(accountId)
      .all<{ organization_id?: unknown; role?: unknown }>();
    const submissionRecordsPromise = listEventScopedJson(
      this.#submissions,
      "Answers JSON",
      eventId,
    ...
    const rosterRecordsPromise = listEventScopedJson(this.#roster, "Members JSON", eventId);
    const decisionRecordsPromise = listEventScopedJson(this.#decisions, "Metadata JSON", eventId);
    const profileRecordsPromise =
      resources.profiles === true
        ? listEventScopedJson(this.#profiles, "Biography", eventId)
        : Promise.resolve([]);
    const taskRecordsPromise =
      resources.tasks === true
        ? listEventScopedJson(this.#tasks, "Owner JSON", eventId)
        : Promise.resolve([]);
    const assetRecordsPromise =
    ...
    const [
      ...
      submissionRecordsPromise,
      rosterRecordsPromise,
      decisionRecordsPromise,
      profileRecordsPromise,
      taskRecordsPromise,
      assetRecordsPromise,
    ]);
    if (event === undefined || event.id !== eventId) return null;
    const tenantId = resolvedOrganizationId(event);
    if (tenantId === undefined) return null;
    const membershipRows = Array.isArray(membershipResult.results) ? membershipResult.results : [];
    const membership = membershipRows.find(
      (candidate) =>
        candidate.organization_id === tenantId &&
        (candidate.role === "owner" || candidate.role === "admin"),
    );

    const role =
      membership?.role === "owner" ? "owner" : membership?.role === "admin" ? "admin" : null;
    if (membership === undefined || membership.organization_id !== tenantId || role === null) {
      return null;
    }
    ...
    const decisions = portalDecisionProjections(
      decisionRecords.filter(
        (record) =>
          eventReference(record) === eventId && matchesOrganizationScope(record, tenantId, true),
      ),
    );
    ...
    const profiles = profileRecords
      .filter(
        (profile) =>
          profile.id === `speaker-profile:${eventId}:${profile.participantId}` &&
...
/** CFP file assets backed by the D1 speaker asset repository and R2 bytes. */
...
interface EvaluationAcceptanceSpeakerRepository extends SpeakerRepository {
  ensureAcceptedSubmission?(input: {
    readonly submission: Submission;
    readonly updatedAt: string;
  }): Promise<SpeakerSubmission>;
  ensureProfile?(input: {
  ...
  ensureVerifiedSpeakerGrant?(input: {
    readonly organizationId: string;
    readonly eventId: string;
    readonly participantId: string;
    readonly email: string;
...
export class AirtableEvaluationAcceptanceHandoff implements EvaluationAcceptanceHandoff {
  ...
  async #ensureAcceptedSubmission(submission: Submission, updatedAt: string): Promise<void> {
    await this.#speakers.ensureAcceptedSubmission?.({ submission, updatedAt });
  }
  ...
  async #ensureSpeakerGrants(
    ...
    await Promise.all(
      submission.participants.map(async (participant) => {
        ...
        const provisioned =
          this.#speakers.ensureVerifiedSpeakerGrant === undefined
            ? await this.#ensureD1SpeakerGrant(input, participant.id, email)
  ...
  async #ensureD1SpeakerGrant(
    input: EvaluationAcceptanceHandoffInput,
    participantId: string,
    email: string,
  ): Promise<boolean> {
    const profile = await this.#speakers.getProfile(input.eventId, participantId);
    ...
    await this.#database
      .prepare(
        `INSERT INTO speaker_grants
           (organization_id, speaker_profile_id, user_id, created_at, revoked_at)
         VALUES (?, ?, ?, ?, NULL)
         ON CONFLICT (organization_id, speaker_profile_id, user_id)
         DO UPDATE SET revoked_at = NULL`,
      )
      .bind(input.tenantId, profile.id, user.id, input.decidedAt)
      .run();
    return true;
  }
