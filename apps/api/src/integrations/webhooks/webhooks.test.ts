...
function principal(organizationId = "org-1", scopes: Array<"webhooks:read" | "webhooks:write"> = ["webhooks:read", "webhooks:write"]) {
...
describe("webhook fan-out and delivery", () => {
  it("is idempotent per subscription and event", async () => {
    const repository = new InMemoryWebhookRepository([], { clock: { now: () => now }, idFactory: (prefix) => `${prefix}_${prefix === "whs" ? "subscription" : "delivery"}` });
    ...
    expect((await dispatcher.fanOut(event)).map((delivery) => delivery.id)).toEqual(["whd_delivery"]);
    expect((await dispatcher.fanOut(event)).map((delivery) => delivery.id)).toEqual(["whd_delivery"]);
  ...
  it("delivers successfully and does not duplicate completed rows", async () => {
    ...
    const deliveries = await new WebhookDispatcher(repository, { clock: { now: () => now } }).fanOut(event);
  ...
  it("schedules bounded retries then records terminal exhaustion", async () => {
    ...
    const dueWorker = new WebhookDeliveryWorker(repository, transport, { clock: { now: () => due }, maxAttempts: 2, initialRetryDelayMs: 100, maxRetryDelayMs: 100 });
...
describe("webhook subscription routes", () => {
  it("enforces tenant/scope and redacts signing secrets", async () => {
    ...
    expect(
      (await forbidden.request("/api/v1/organizations/org-1/webhooks")).status,
    ).toBe(403);
  });
});
