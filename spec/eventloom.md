...
- Existing provider-side fields from retired mappings may remain in connected bases; Eventloom ignores them and never treats their values as authoritative.
...
Accelevents is a separate external event platform. Its one-way integration item
was struck from the competition brief and is outside the evaluator requirements,
so it is not a supported Eventloom runtime feature. No Accelevents credentials,
setup, publication, synchronization, or release gate is required. Historical
adapter code or references do not change that classification.
...
9. Organization creation is an operator or managed-control-plane provisioning action, never an action inside an existing organization workspace or public member API. Self-hosted deployments may bootstrap the first organization with an operator credential; additional organizations use the authenticated provisioning contract. `/work` presents one explicit destination per authorized organization so a multi-organization account can choose its workspace without signing in again.
10. Managed deployments enforce provider-neutral organization entitlements in the product API. Organization provisioning persists the owner membership and entitlement atomically, and event creation requires the `events.create` capability while reserving non-retired event capacity in the same authoritative D1 transaction as the event record. Organizer seat limits apply to owner/admin memberships and pending invitations. Self-hosted deployments use the same product boundary with an unrestricted local policy.
...
- **partial:** Calendar timezone migration remains a known gap and has no automatic draft migration, revalidation, and republish workflow. Stable DST-specific API error responses are source-present and locally tested, but deployed and staging evidence remains absent. Private-upload policy requests malware scanning, but no supported file-scan Queue consumer completes that boundary. The production reminder Cron can also return early on invalid runtime configuration without durable failure evidence. These paths require release re-verification.
