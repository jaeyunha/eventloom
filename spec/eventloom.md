...
10. Managed deployments enforce provider-neutral organization entitlements in the product API. Organization provisioning persists the owner membership and entitlement atomically, and event creation reserves event capacity in the same authoritative D1 transaction as the event record. Self-hosted deployments use the same product boundary with an unrestricted local policy unless the operator configures another policy.
