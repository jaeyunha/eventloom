...
## 2026-08-11 run: 41-check manual-results workflow

The one-to-one map for the 41 pending IDs generated by run
`2026-08-11T14-11-08` is in
[`docs/llm-judge-runs.md`](llm-judge-runs.md#exact-id-to-artifact-to-report-map).
The set includes manual, auto-partial, and automated items routed to follow-up
after `cannot_judge`; it is not a permanent checklist for every run. Preserve:
...
Before entering a result for that run, verify the exact 41-key set from the table. Each
record is keyed by the rubric ID (for example,
`manual-results.json["CFP-14"]`), not by a scenario ID such as `CFP-S4`.
Every record must have a non-placeholder `verdict` and non-empty `notes` that
state the reviewer/role, UTC `reviewed_at`, observed boundary, limitations, and
relative redacted `evidence_refs` under `manual/<CHECK-ID>/`. Do not put
passwords, magic links, inbox contents, API keys, or private payloads in those
artifacts.
...
| Partial | `partial` | UI/automated half or another material part works, but the manual criterion is incomplete; remains a limitation. |
...
| Blocked/insufficient | `cannot_judge` | Do not put this in a finalized result; leave it pending and record why. |

The current evaluator finalizer is a fold/rescore step, not a complete
validator. From the evaluator checkout, run:

```bash
pnpm run finalize -- --run <run-dir>
```

It consumes recognized pending keys from `manual-results.json`, updates the
matching item in `report.json`, regenerates `report.html`, and rewrites the
pending checklist. It currently warns/ignores unknown or already-finalized
keys, leaves missing/invalid/placeholder/`cannot_judge` rows pending, and does
not detect duplicate JSON object keys. QA must therefore perform the
pre-finalize gate itself: reject missing, unknown, duplicate, placeholder, or
non-passing required rows before treating the command as successful. For this
run, require all 41 rows to be `pass` for release; `partial`, `fail`, and
`not_found` are visible non-passing limitations. For any rerun, validate the
newly generated pending queue instead of copying the 41-ID set. After
finalization, inspect `report.json` and `report.html`: `manualPending` must be
zero, `scoreWithheld` false, and each row's cited artifact must exist, be
redacted, and match the same candidate, environment, and UTC reviewer record.

The current repository has no completed 41-row manual verification in this
documentation change. Do not mark any row pass or present a generated report
as release evidence until those observations and artifacts exist.

## Manual evidence result-row checklist

For each of this run's 41 rows, retain the existing evidence-row fields above and add:

| Field | Required value |
| --- | --- |
| Check ID | One ID from this run's mapped pending set, exactly once |
